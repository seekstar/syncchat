...
bool odbc_login(ostream& errInfo, const char* dataSource, const char* user, const char* passwd) {
    SQLRETURN ret;
    ...
    if(!SQL_SUCCEEDED(ret))
    {
        errInfo<<"AllocEnvHandle error!"<<endl;
        goto ERR_serverenv;
    }
    ...
    if(!SQL_SUCCEEDED(ret))
    {
        errInfo<<"AllocDbcHandle error!"<<endl;
        goto ERR_HANDLE_DBC;
    }
    ...
    if(!SQL_SUCCEEDED(ret))
    {
        errInfo<<"SQL_Connect error!"<<endl;
        goto ERR_CONNECT;
    }
...
ERR_HANDLE_STMT:
    ...
    if(SQL_SUCCESS!=ret&&SQL_SUCCESS_WITH_INFO!=ret) {
        errInfo<<"disconnected error!"<<endl;
        return 1;
    }
ERR_CONNECT:
    //释放连接句柄
...
bool odbc_logout(ostream& errInfo) {
    SQLRETURN ret;

    //释放语句句柄
    ret=SQLFreeHandle(SQL_HANDLE_STMT,serverhstmt);
    if(SQL_SUCCESS!=ret && SQL_SUCCESS_WITH_INFO != ret) {
        errInfo<<"free hstmt error!"<<endl;
        return 1;
    }
    //断开数据库连接
    ret=SQLDisconnect(serverhdbc);
